...
void EarthRad(double lat, double *Rew, double *Rns) {
  ...
  (*Rew) = EARTH_RADIUS / sqrt_denom; // Transverse (along East-West)
  (*Rns) = EARTH_RADIUS * (1 - ECC2) / (denom * sqrt_denom); // Merdian (along North-South)
}
...
double Gravity(Vector3d p_E){
	...
	Vector3d p_LLA = E2D(p_E);

	g0 = 9.7803253359 * (1+0.001931853 * sin(p_LLA(0)) * sin(p_LLA(0))) / sqrt(1-(ECC2 * sin(p_LLA(0))) * (ECC2*sin(p_LLA(0))));
...
double Geocentric_Radius(Vector3d p_E){
	  ...
	  R_geo = EARTH_RADIUS/sqrt(1-(ECC2 * sin(p_LLA(0))) * (ECC2*sin(p_LLA(0)))) * sqrt(cos(p_LLA(0)) * cos(p_LLA(0))+(1-ECC2 * ECC2) * (1-ECC2 * ECC2) * sin(p_LLA(0)) * sin(p_LLA(0)));
